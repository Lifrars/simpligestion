...
if ($indicador == "1") {
    try {
        ...
        $query2 = "WITH VotosPorTitulo AS (
            SELECT v.id, v.titulo, o.descripcion, v.descripcion AS descripcion2, e.id_opcion, v.estado,
          ...
          SELECT id, titulo, descripcion2, descripcion, id_opcion, Total, estado
          FROM VotosPorTitulo
          WHERE rn = 1;";
...
if ($indicador == "3") {
    try {
        ...
        $query = 'INSERT INTO votaciones (titulo, descripcion, estado) VALUES (:titulo, :descripcion, 1)';
        ...
        if ($stmt->execute()) {
            ...
            if (!is_array($opciones)) {
                $opciones = explode(',', $opciones); // Suponiendo que las opciones están separadas por comas
            }
            ...
            header('Content-Type: application/json');
            echo json_encode(array('rta' => $rta, 'code' => $id_votacion, 'query' => $query));
        } else {
            $rta = "error";
            header('Content-Type: application/json');
