...
<body class="form">
						...
						<form class="text-left" id="loginforgot" style="display:none">
							<div class="form">
								<div id="username-field" class="field-wrapper input">
									...
									<input id="emailu" name="emailu" type="text" class="form-control"
										placeholder="email@dominio.com">
								</div>

								<div class="d-sm-flex justify-content-between">
									<div class="field-wrapper">
										<button id="btnForgot" class="btn btn-primary">Recover Access</button>
									</div>
								</div>
							</div>
						</form>

						<div class="text-center"><img id="imgok" src="../imgs/gloading.gif" style="display:none;"
								width="150px"></div>

						<h2 id="lblloading" style="display:none;color:black; text-align:center">Requesting Access...
						</h2>
					</div>
				</div>
			</div>
	...
	<script>
		...
		$("#btnForgot").click(function () {
			...
			var correo = document.getElementById('emailu').value;
			console.log(correo)
			let ladata = new FormData();
			ladata.append('correo', correo);
			ladata.append('ind', '9');

			fetch('loginback2.php', {
				method: 'POST',
				body: ladata,
			})
				.then(response => {
				...
				.then(data => {
					if (data.rta === 'ok') {
						Swal.fire({
							...
							text: 'Se ha enviado un código temporal a tu correo electrónico.',
						}).then(() => {
							window.history.back();
						});
					} else if (data.rta === 'correo_no_encontrado') {
						Swal.fire({
							icon: 'warning',
							title: 'Correo no encontrado',
							text: 'El correo ingresado no está registrado en el sistema.',
						});
					} else if (data.rta === 'correo_requerido') {
						Swal.fire({
							icon: 'warning',
						...
						Swal.fire({
							icon: 'error',
							title: 'Error',
							text: 'Ocurrió un problema al procesar tu solicitud. Inténtalo nuevamente.',
						});
					}
				})
				.catch(error => {
