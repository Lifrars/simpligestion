...
async function leerdatos(id) {
  // Limpiar los campos y cargar datos auxiliares
  limpiarCampos();
  traerPerfiles();
  traerTorres();
  ...
  fetch('backend/usuariosBackend.php', {
    ...
    .then(data => {
      ...
      document.getElementById('sel_perfil').value = usuario.idPerfil;
      document.getElementById('id_residencia').value = usuario.id_residencia;
...
function limpiarCampos() {
  ...
  document.getElementById('id_residencia').value = '0';
}
...
function lisrarUsuarios(ind, inicio, nroreg) {
  ...
  fetch('backend/usuariosBackend.php', {
    ...
    data.rta2.forEach(function (item) {
      ...
      htmlTags += '     <td>' + item.nombre_perfil + '</td>';
      htmlTags += '     <td>';
      htmlTags += '         <a class="me-3" id="detail" onclick="detailUsuario(' + item.id + ');" name="detail" data-validar="SI"';
      htmlTags += '         data-tipo="TABLA_MODAL" data-apodo="Detalles Producto" style="cursor: pointer;"><img src="//ppi.miclickderecho.com/plantilla/assets/img/icons/eye.svg" alt="img"></a>';
      htmlTags += '         <a class="me-3" id="editProducto" onclick="leerdatos(' + item.id + ');" name="editProducto"';
      htmlTags += '         data-validar="SI" data-tipo="TABLA_MODAL" data-apodo="Editar Producto" style="cursor: pointer;"><img src="//ppi.miclickderecho.com/plantilla/assets/img/icons/edit.svg" alt="img"></a>';
      htmlTags += '         <a class="me-3" id="deleteproducto" onclick="deleteUsuario(' + item.id + ');" name="deleteproducto"';
      htmlTags += '         data-validar="SI" data-tipo="TABLA_MODAL" data-apodo="Eliminar Producto" style="cursor: pointer;"><img src="//ppi.miclickderecho.com/plantilla/assets/img/icons/delete.svg" alt="img"></a>';
      htmlTags += '     </td>';
...
function validarCampos() {
  ...
  var id_residencia = document.getElementById('id_residencia');

  var campos = [documento, telefono, nombre, correo, perfil, id_residencia];
  var nombresCampos = ['Documento', 'Teléfono', 'Nombre', 'Correo', 'Perfil', 'Torre'];
...
function Save_usuario() {
  ...
  var id_residencia = document.getElementById('id_residencia').value;
  ...
  ladata.append('id_residencia', id_residencia);
  ladata.append('ind', 3);
  ...
  fetch('backend/usuariosBackend.php', {
    ...
    .then(data => {
      ...
      } else if (data.rta === 'residencia_ocupada') {
        Swal.fire({
          icon: 'warning',
          title: 'Residencia ocupada',
          text: 'Ya hay un propietario asociado a esa residencia. No se puede asignar otro.',
        });
      } else {
        Swal.fire({
          icon: 'error',
...
//Traer los torres del sistema
function traerTorres() {
  let ladata = new FormData();
  ladata.append("ind", "8");
  ...
  fetch('backend/usuariosBackend.php', {
    ...
    .then(data => {
      ...
      let htmlTags = '<option value="0">Seleccionar</option>';
      data.rta.forEach(function (item) {
        htmlTags += `<option value="${item.id_residencia}">${item.nombre_residencia}</option>`;
      });
      $("#id_residencia").append(htmlTags);
    })
    .catch(function (error) {
...
function detailUsuario(id) {
  ...
  fetch('backend/usuariosBackend.php', {
    ...
    .then(data => {
      ...
      document.getElementById('showResidencia').textContent = data.rta.nombre_residencia_formateada;
